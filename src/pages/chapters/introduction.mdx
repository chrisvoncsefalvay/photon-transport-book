...
The first two are properties of a transmission calculation. The remaining quantities involve the detector and, eventually, the processing pipeline. The distinction becomes immediate when we differentiate. Increasing the thickness of a homogeneous attenuating slab reduces primary transmission. Its negative logarithm increases. A display transformation may reverse the visual polarity again. A derivative's sign makes sense only after we know which quantity was differentiated. [Appendix A.5](/appendix/x-ray-theory/#a5-attenuation-along-a-ray) develops the slab calculation, and [Appendix A.15](/appendix/x-ray-theory/#a15-what-these-distinctions-mean-for-differentiable-modelling) follows its consequences for differentiable models.
...
The energy integral is already doing important work. Photons at different energies encounter different attenuation and produce different detector responses. In general, the integral cannot be replaced by an exponential evaluated at one universal effective energy. The source spectrum, transmitted material and detector weighting all matter. [Appendix A.3](/appendix/x-ray-theory/#a3-what-the-x-ray-source-produces) introduces the source, while [Appendix A.6](/appendix/x-ray-theory/#a6-why-a-clinical-exposure-is-not-generally-a-single-line-integral) explains why a clinical exposure is not usually one line integral. <Citation keys={["alvarez1976"]} />

A finite focal spot or detector aperture requires averaging contributions over different paths. Motion requires integration over time. Spatial signal spread requires a detector operator that mixes neighbouring locations. These effects cannot all be hidden inside an object-independent open-beam spectrum. Their geometrical and spatial consequences are developed in [Appendix A.7](/appendix/x-ray-theory/#a7-projection-geometry-magnification-and-blur), [Appendix A.9](/appendix/x-ray-theory/#a9-what-a-detector-measures) and [Appendix A.10](/appendix/x-ray-theory/#a10-resolution-is-more-than-pixel-spacing).
...
The scientific implementation lives under `python/dpt/`. The modules in [Table 1.2](#table-1-2) are present in the accompanying library, and the chapter order describes how we arrive at their design. You can follow that argument from transmission onwards, or use the map to find the operation needed for an experiment.
...
| `dpt.validation`, `dpt.experiments`                                        | Check operators against independent references and record the inputs, source and outputs of an execution.                          | Throughout the implementation.                                                                       |
...
With pose recovery working, [Chapter 7](/chapters/same-pose-different-x-ray/) introduces controlled exposure, gain and offset changes, then examines how a pose fit responds to those changes and to errors in its scene model. [Chapter 8](/chapters/spectral-transport-detector/) then implements energy-dependent materials, spectral integration and detector response. Spectral experiments require those operators, and scatter experiments must also wait for Chapter 9’s transport implementation. The distinctions in [Appendix A.13](/appendix/x-ray-theory/#a13-same-pose-different-x-ray) will become modelling choices rather than background reading.
