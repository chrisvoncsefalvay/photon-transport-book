...
The explicit `field_domain="nonnegative"` option instead accepts finite, nonnegative dimensionless equivalent-basis coefficients without upper or sum caps. Those coefficients are not volume fractions: their normalisation belongs in the supplied basis attenuation, in $\mathrm{mm}^{-1}$. [Chapter 12](/chapters/reconstruction/#figure-acquired-spectral) uses that domain for the acquired PMMA/Al-equivalent example.
