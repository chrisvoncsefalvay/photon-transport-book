...
Coordinates and quadrature accumulators are FP64; samples and optical depths
are FP32. Numerical acceptance and performance must be established on CUDA;
...
class ProjectionSpec:
    """Fixed-count midpoint discretisation and explicitly supported active inputs.
...
def project_optical_depth(
    ...
    *,
    ...
    values (R_WO row-major, t_WO in mm). out_L is flat FP32, row-major detector
    order. A checked call synchronises domain diagnostics before writing and
    ...
    ctx = workspace.context
    ctx.array(out_L, "out_L", dtype=ctx.wp.float32, shape=(workspace.geometry.pixels,))
...
def projection_vjp(
    ...
    *,
    ...
    ctx.array(adj_L, "adj_L", dtype=wp.float32, shape=(workspace.geometry.pixels,))
    ...
    wp.launch_tiled(
        workspace._kernels.get_vjp(out_mu is not None, components == 12, target is not None),
        dim=count,
...
def projection_pose_sensitivities(
    ...
    *,
    ...
    seeds are FP32 ``(pixels,)`` and output is FP64 ``(6*pixels,)``. Ones export
    ...
    ctx.array(depth_seeds, "depth_seeds", dtype=wp.float32, shape=(pixels,))
    ...
    wp.launch_tiled(
        workspace._kernels.get_vjp(False, False, True, per_ray=True),
