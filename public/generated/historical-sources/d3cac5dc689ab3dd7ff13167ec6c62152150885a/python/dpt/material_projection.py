...
values in millimetres. Each basis field is a volume fraction. Fractions are
...
def prepare_material_projection(
    ...
    *,
    ...
    ctx.array(out_paths, "out_paths", dtype=wp.float32, shape=(materials * geometry.pixels,))
    reads = [("fields", fields)]
    writes = [("out_paths", out_paths)]
    if adj_paths is not None:
        ctx.array(adj_paths, "adj_paths", dtype=wp.float32, shape=(materials * geometry.pixels,))
...
def _validate(workspace: MaterialProjectionWorkspace, *, seeds: bool) -> None:
    ...
    if seeds:
        ctx.wp.launch(
            workspace.kernels.validate_seeds,
