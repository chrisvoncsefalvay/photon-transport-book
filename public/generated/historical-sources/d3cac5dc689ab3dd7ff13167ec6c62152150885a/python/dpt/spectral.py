...
uploads; evaluations accept caller-owned flat binary32 arrays. Reverse products
...
class SpectralWorkspace:
    ...
    def check_status(self) -> None:
        ...
        if code & 2:
            raise NumericalError("spectral output or gradient cannot be represented in binary32")
        ...
        for values, nonnegative in arrays:
            if values.size:
                self.context.wp.launch(
                    self._kernels.check_nonnegative if nonnegative else self._kernels.check_finite,
                    dim=values.size,
                    inputs=[values],
    ...
    def validate_inputs(
        ...
        *,
        ...
        if paths is not None:
            ctx.array(paths, "paths", dtype=ctx.wp.float32, shape=(spec.materials * pixels,))
...
def prepare_spectral(
    ...
    *,
    ...
    return SpectralWorkspace(spec, max_pixels, context, kernels, status, empty, partials)
...
def _inputs(
    ...
    size = ctx.array(paths, "paths", dtype=ctx.wp.float32)
...
def spectral_signal(
    ...
    *,
    ...
    Inputs/outputs are flat contiguous binary32 CUDA arrays. ``weights`` are
    ...
    ctx.array(out_mean, "out_mean", dtype=ctx.wp.float32, shape=(pixels,))
    ...
    *,
    ...
    for name, output, active, length in destinations:
        if output is not None:
            ...
            ctx.array(output, f"out_grad_{name}", dtype=ctx.wp.float32, shape=(length,))
    ...
    with ctx.scope():
        ...
        if pixels and (out_grad_paths is not None or pixel_weights or pixel_response):
            ctx.wp.launch(
                ...
                outputs=[
                    out_grad_paths if out_grad_paths is not None else workspace._empty,
            ...
            if pixels:
                partial_kernel = (
                    workspace._kernels.get_coefficient_partials(
                        spec.materials, spec.energies, spec.shared_weights, spec.shared_response
                    )
                    if kind == 2
...
def spectral_bin_counts(
    ...
    *,
    ...
    ctx.array(
        out_bin_counts, "out_bin_counts", dtype=ctx.wp.float32, shape=(spec.energies * pixels,)
    )
