        ...
        *,
        ...
        with self.context.scope():
            ...
            self._paths = wp.empty(
                self.materials * maximum_pixels, dtype=wp.float32, device=self.device
            )
            self._path_seed = wp.empty_like(self._paths)
            ...
            self._image_seed = wp.empty(maximum_pixels, dtype=wp.float32, device=self.device)
            ...
            self._objective = prepare_objective(
                ...
                if signal_wls
                else ObjectiveSpec(kind="poisson", domain="counts"),
            ...
            for view, counts, weights, response, objective_weights, valid in prepared:
                ...
                for name, fields in (("accepted", self.fractions), ("trial", self.trial)):
                    item[name] = prepare_material_projection(
                        grid,
                        view.geometry,
                        ProjectionSpec(samples_per_ray, active_pose=False, active_volume=True),
                ...
                for channel in range(counts.shape[0]):
                    row = {
                        ...
                        "mean": wp.empty(pixels, dtype=wp.float32, device=self.device),
            ...
            for view in self.views:
                ...
                for channel in view["channels"]:
                    ...
                    if gradient:
                        ...
                        self.wp.launch(
                            self.kernels.add_paths,
                            dim=self.materials * view["pixels"],
                            inputs=[view["channel_seed"], view["path_total"]],
                            stream=self.stream,
                            record_tape=False,
                        )
                if gradient:
    ...
    def solve(
        ...
        for iteration in range(policy.iterations):
            ...
            if mapping <= tolerance:
                ...
                break
            found = False
            for backtracks in range(policy.maximum_backtracks):
                if step == 0:
                    break
                try:
                    _, slope = self.projected_trial(step)
                    if slope >= 0:
