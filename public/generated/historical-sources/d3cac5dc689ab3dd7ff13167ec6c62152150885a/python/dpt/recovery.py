    ...
    def __init__(
        ...
        *,
        ...
        self.projection = prepare_projection(
            problem.grid,
            problem.geometry,
            ProjectionSpec(problem.samples_per_ray),
            device=device,
            stream=stream,
        )
        ...
        with ctx.scope():
            ...
            self.optical_depth = wp.empty(count, dtype=wp.float32, device=ctx.device)
            self.prediction = wp.empty(count, dtype=wp.float32, device=ctx.device)
            self.image_seed = wp.empty(count, dtype=wp.float32, device=ctx.device)
            self.depth_seed = wp.empty(count, dtype=wp.float32, device=ctx.device)
